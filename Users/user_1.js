...
async function run() {
    while(true){
        ...
        if (input == 1){
            ...
            let key = `${grpIP}:${grpPort}`;
            groupMapping.set(key, [groupSocket,UserName]);
            console.log("------------------------------------------------------------------------------");
        ...
        else if (input==4)
        {
            ...
            groupSocket = groupMapping.get(key)[0];
            UserName = groupMapping.get(key)[1];
            groupConsoleInput = 0;
